...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
    ...
    let x = coordX1, y = coordY1;

    function animateLineDrawing() {
        const animationLoop = requestAnimationFrame(animateLineDrawing);
        c.clearRect(0, 0, 608, 608); // Clears canvas for the next frame
        c.beginPath();
        ...
        if (x < coordX2) { x += 10; if (x > coordX2) x = coordX2; }
        if (x > coordX2) { x -= 10; if (x < coordX2) x = coordX2; }
        
        // Move Y toward the target
        if (y < coordY2) { y += 10; if (y > coordY2) y = coordY2; }
        if (y > coordY2) { y -= 10; if (y < coordY2) y = coordY2; }
